...
$BODY$
 ...
 BEGIN
     IF TG_OP = 'DELETE' THEN
         ...
         DELETE FROM user_session WHERE user_id = OLD.user_id;
...
CREATE OR REPLACE FUNCTION fn_user_session() RETURNS TRIGGER AS
$BODY$
 DECLARE
 BEGIN
     IF TG_OP = 'DELETE' THEN
         DELETE FROM user_activity WHERE session_id = OLD.user_id;
         RETURN OLD;
     END IF;
     IF TG_OP = 'INSERT' THEN
         RETURN NEW;
