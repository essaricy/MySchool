CREATE OR REPLACE FUNCTION wavefire.copy_user_privileges(from_user_id INT, to_user_ids INT[]) RETURNS VOID AS
$BODY$
DECLARE
  -- declarations
...
BEGIN
  ...
  SELECT ref_user_type_id INTO from_user_type_id FROM wavefire.users WHERE user_id = from_user_id;
  ...
  LOOP
    ...
    SELECT ref_user_type_id INTO to_user_type_id FROM wavefire.users WHERE user_id = to_user_id;
  ...
  DELETE FROM wavefire.user_access WHERE user_id = ANY (to_user_ids);

  -- Insert records
  RAISE NOTICE 'Copying privileges to the users % ', to_user_ids;
  FOREACH to_user_id IN ARRAY to_user_ids
  LOOP
    INSERT INTO wavefire.user_access (SELECT to_user_id, function_id, can_view, can_create, can_update, can_delete FROM wavefire.user_access WHERE user_id = from_user_id);
