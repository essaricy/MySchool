...
$BODY$
  ...
  BEGIN
    ...
    FOR curUserType IN 
      ...
      from wavefire.ref_user_type
    LOOP
      DELETE FROM wavefire.default_user_access WHERE ref_user_type_id = curUserType.user_type_id;

      FOR curRecord IN 
        SELECT ref_module.module_id, ref_module.module_name, function.function_id
        from wavefire.FUNCTION
        inner join wavefire.ref_module on ref_module.module_id= function.ref_module_id
      LOOP
        ...
        INSERT INTO wavefire.DEFAULT_USER_ACCESS (REF_USER_TYPE_ID, FUNCTION_ID, CAN_VIEW, CAN_CREATE, CAN_UPDATE, CAN_DELETE)
