...
/**
 * The Class JBossServerAgent.
 */
@Component
public class JBossServerAgent extends WebServerAgent {

    /* (non-Javadoc)
     * @see com.myschool.infra.webserver.agent.WebServerAgent#getDataSource(java.util.Properties)
     */
    @Override
    public DataSource getDataSource(Properties dataSourceProperties)
            throws AgentException {
        try {
            String dataSourceName = (String) dataSourceProperties.get(DataSourceConstants.JDBC_DATASOURCE_NAME);
            Context context = new InitialContext();
            return (DataSource) context.lookup("java:/" + dataSourceName);
